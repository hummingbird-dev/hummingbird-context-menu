(function($){
    ...
    var popover_list = '<div class="table-responsive text-center" style="cursor:pointer;"><table class="table table-striped table-condensed"><tbody>';
	...
	var poplist = $(".hummingbird-popover-converter").children("li");
	poplist.hide();
	//console.log(poplist)
	...
	$.each(poplist, function(i,e) {
	    ...
	    if (typeof(popover_lists[index]) == "undefined") {
		popover_lists[index] = '<div class="table-responsive text-left" style="cursor:pointer;"><table id="' + 'tab_' + index  + '" class="table table-striped table-condensed popover_table"><tbody>';
		popover_lists[index] = popover_lists[index] + '<tr class="' + endnode  + '"  id="' + layer  + '"><td  style="white-space:nowrap;border:none;">' + popText + '</td><td  style="white-space:nowrap;border:none;">' + caret_right + '</td></tr>'
	    } else {
		popover_lists[index] = popover_lists[index] + '<tr class="' + endnode  + '"  id="' + layer  +'"><td  style="white-space:nowrap;">' + popText + '</td><td  style="white-space:nowrap">' + caret_right + '</td></tr>'
	    }
	...
	//close the tables
	$.each(popover_lists,function(i,e) {
	    popover_lists[i] = popover_lists[i] + '</tbody></table></div>';
	});
	...
	if (typeof(methodName) == "undefined" ) {
		...
		bindpoint.on({
			...
			if (e.which == mouseButton) {
			    ...
			    var k = 1
			    $('<a  class="hummingbird-popover" href="#" id="hummingbird-popover-'+ k  +'" title="<b>Options</b>" data-content="Some content inside the popover">&nbsp;</a>').css({
	    			position: 'absolute',
	    			top: screen_mouse_y,
	    			left: screen_mouse_x,
				zIndex: 1000000
			    ...
			    $('#hummingbird-popover-' + k).attr("data-content",popover_lists["0"]);
				...
				these_trs.hover(
					...
					if (typeof(str) !== "undefined") {
						...
						console.log("index= " + index);
						var title = ""
						$(this).popover({html:true,placement:"auto right",content:popover_lists[index]});
						$(this).popover("show");
